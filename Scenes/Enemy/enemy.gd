...
const DEATH = preload("res://Scenes/UI/death.tscn")
...
func _ready() -> void:
	...
	GameManager.pause.connect(stop_all)
...
func die() ->void:
	spawn_particle()
	var sound_fx = DEATH.instantiate()
	get_tree().get_first_node_in_group("BulletHolder").call_deferred("add_child",sound_fx)
